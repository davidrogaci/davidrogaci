# 👋 Hi, I'm David Rodriguez

### 🚀 Front-end Web Developer

I'm a web developer with experience in creating unique and engaging experiences. Currently, I have completed my Front-end Web Development degree and am in the process of perfecting my skills and acquiring new competencies in the web development world.

![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=for-the-badge&logo=typescript&logoColor=white)
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![Angular](https://img.shields.io/badge/Angular-DD0031?style=for-the-badge&logo=angular&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)

## 📍 Personal Information

- 🌐 **Location:** Montevideo, Uruguay
- 🎓 **Education:** Front-end Web Developer
- 💼 **Professional Goal:** Become a complete Front-end Web Developer
- 🕒 **Availability:** Monday to Friday from 09:00 to 18:00

## 💻 Work Experience

### Front-end Developer Freelance

**2022 - Present**

- Development of responsive and accessible user interfaces
- Implementation of reusable components in React and Angular
- Performance and SEO optimization
- Collaboration with multidisciplinary teams

## 🏆 Achievements

- 🏅 Front-end Web Development Certification
- 📈 40% improvement in application performance
- 🎯 Successful implementation of web projects
- 🌟 Recognition for best coding practices

## 🛠️ Technical Skills

### Programming Languages

- **JavaScript/TypeScript:** Modern web application development
- **HTML5/CSS3:** Responsive and accessible interface creation

### Frameworks and Libraries

- **React:** SPA development and reusable components
- **Angular:** Enterprise application building
- **Material-UI/Tailwind:** Interface design

### Tools and Platforms

- **Git/GitHub:** Version control and collaboration
- **VS Code:** Development and debugging
- **Figma:** Design and prototyping

## 📊 Featured Projects

### [E-commerce Rodriguez](https://github.com/davidrogaci/ecommerce-rodriguez)

- **Technologies:** React, Redux, Node.js, MongoDB
- **Features:** Shopping cart, authentication, payments
- **Results:** 30% increase in conversions, 40% performance improvement
- [Live Demo](https://ecommerce-rodriguez.vercel.app)

### [Jordan 1 Retro Spider](https://github.com/davidrogaci/Jordan-1-Retro-Spider)

- **Technologies:** Angular, Firebase, SCSS
- **Features:** Interactive gallery, reservation system
- **Results:** 50% increase in engagement, 25% in conversions
- [Live Demo](https://jordan-1-retro-spider.vercel.app/)

## 📚 Certifications

- CoderHouse HTML and CSS [Certificate](https://pub.coderhouse.com/legacy-certificates/6505f90a7dab041f9ddb11bf?lang=es)
- CoderHouse JavaScript [Certificate](https://pub.coderhouse.com/legacy-certificates/65d100054d57812ddf30e80f?lang=es)
- CoderHouse React(Vite) [Certificate](https://pub.coderhouse.com/legacy-certificates/66b0bcb13deda7fe3bacf37d?lang=es)

## 📫 How to Reach Me

- **Email:** [davidrogaci5@gmail.com](mailto:davidrogaci5@gmail.com)
- **LinkedIn:** [linkedin.com/in/davidrogaci](https://linkedin.com/in/davidrogaci)
- **Twitter:** [@davidrogaci](https://twitter.com/davidrogaci)
- **Portfolio:** [davidrogaci.dev](https://davidrogaci.dev)

## 💬 Languages

- **Spanish:** Native
- **English:** Intermediate (B2)

## 📊 GitHub Stats

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=davidrogaci&show_icons=true&theme=radical)

---

Thank you for visiting my profile! I'm open to new opportunities and collaborations. Feel free to contact me to discuss interesting projects.
